...
// Negative constraint patterns for fashion/streetwear context
// n[eé]lk[uü]l[i]? — handles both accented and non-accented Hungarian
// [\s\-]* — allows space, hyphen, or nothing between word and "nélküli"
const _NLK = `n[eé]lk[uü]l[i]?`;
const _SEP = `[\\s\\-]*`; // separator: space, hyphen, or nothing
const NEGATIVE_PATTERNS: Array<[RegExp, string]> = [
  [new RegExp(`log[oó]${_SEP}${_NLK}|log[oó]${_SEP}mentes`, "i"), "logo"],
  [new RegExp(`felirat${_SEP}${_NLK}|felirat${_SEP}mentes`, "i"), "felirat (szöveg/text)"],
  [new RegExp(`minta${_SEP}${_NLK}|minta${_SEP}mentes`, "i"), "minta (print/graphic pattern)"],
  [new RegExp(`grafika${_SEP}${_NLK}|grafika${_SEP}mentes`, "i"), "grafika (graphic print)"],
  [new RegExp(`nyomat${_SEP}${_NLK}|nyomat${_SEP}mentes`, "i"), "nyomat (print)"],
  [new RegExp(`cs[ií]k(os)?${_SEP}${_NLK}`, "i"), "csík (stripe)"],
  [/\bwithout\s+logo\b|\bno[- ]?logo\b|\blogo[- ]?free\b/i, "logo"],
  [/\bwithout\s+(print|graphic|pattern|text|writing)\b|\bno\s+(print|graphic|pattern|text|writing)\b/i, "print/graphic"],
  [/\bplain\b(?!\s*(?:yogurt|water|rice|pasta|flour))/i, "print/graphic/logo (plain/díszítés nélküli)"],
];

/**
 * Detects negative/exclusion constraints from free text.
 * Returns array of human-readable exclusion labels, e.g. ["logo", "print/graphic"]
 */
export function detectNegativeConstraints(freeText: string): string[] {
  if (!freeText) return [];
  const found: string[] = [];
...
function detectExplicitFilters(freeText: string, interests: string[]): string {
  ...
  const negativeConstraints = detectNegativeConstraints(freeText);
  if (negativeConstraints.length > 0) {
    lines.push(
      `- EXCLUSION FILTER: The user explicitly does NOT want: ${negativeConstraints.join(", ")}.\n  → Products that have these attributes MUST go to "also_items" or be omitted entirely.\n  → "items" list MUST ONLY contain products WITHOUT these attributes. Check name + description carefully.`
    );
  }
