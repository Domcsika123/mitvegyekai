...
export interface QuerySignals {
  type: string | null; // first detected type (backward compat)
  types: string[];     // ALL detected types (multi-type support)
...
const TYPE_SYNONYMS: Record<string, string> = {
  ...
  alsonemu: "fehérnemű",
  underwear: "fehérnemű",
  boxer: "férfi fehérnemű",
  "férfi fehérnemű": "férfi fehérnemű",
  "ferfi fehernemu": "férfi fehérnemű",
  "férfi alsónemű": "férfi fehérnemű",
  "ferfi alsonemu": "férfi fehérnemű",
  "női fehérnemű": "női fehérnemű",
  "noi fehernemu": "női fehérnemű",
  "női alsónemű": "női fehérnemű",
  "noi alsonemu": "női fehérnemű",
  lingerie: "női fehérnemű",
  bodysuit: "női fehérnemű",
  ...
  // Phone cases
  "telefontok": "telefontok",
  "mobiltelefon tok": "telefontok",
  "iphone tok": "telefontok",
...
export function parseQuery(query: string): QuerySignals {
  ...
  for (const token of tokens) {
    ...
    // Check type (collect ALL, not just first)
    if (TYPE_SYNONYMS[normalizedToken]) {
      const t = TYPE_SYNONYMS[normalizedToken];
  ...
  // Also check multi-word types (e.g., "kapucnis pulóver") — for any not yet found
  // Use word-boundary regex to prevent "ruha" matching inside "fürdőruha",
  // or "nadrág" matching inside "rövidnadrág", etc.
  for (const [synonym, canonicalType] of Object.entries(TYPE_SYNONYMS)) {
    if (foundTypes.includes(canonicalType)) continue;
    const normalizedSyn = normalizeText(synonym);
    const regex = new RegExp(`\\b${normalizedSyn}\\b`, "i");
    if (regex.test(normalized)) {
      foundTypes.push(canonicalType);
    }
  }
...
export function signalsSummary(signals: QuerySignals): string {
  ...
  if (signals.types.length) parts.push(`types=[${signals.types.join(",")}]`);
